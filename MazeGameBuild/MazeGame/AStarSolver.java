...
public class AStarSolver implements MazeSolverStrategy {
    private boolean[][] visited;
    private MazeStepListener stepListener;
    private int rows, cols;
    private int moveCounter; // Move counter to track number of steps

    public AStarSolver(MazeStepListener listener) {
    ...
    public List<Point> solve(int[][] maze, Point start, Point end) {
        ...
        gScore.put(start, 0);
        openList.add(new Node(start, 0, manhattanDistance(start, end)));
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (isValidMove(maze, next)) {
                    ...
                    if (tentativeGScore < gScore.getOrDefault(next, Integer.MAX_VALUE)) {
                        ...
                        int f = tentativeGScore + manhattanDistance(next, end);
                        openList.add(new Node(next, tentativeGScore, f));
                    }
                }
            }

            // Increment the moveCounter after processing the current node
            moveCounter++;
        }
